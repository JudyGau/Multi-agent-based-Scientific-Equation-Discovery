...
# run_problem_file <problem_name> <train.csv 路径> <backgrounds/*.txt 路径> #   与 run_problem 相同，但背景词从文件读取：backgrounds/*.txt 是规范源， #   改 txt 即刻生效，脚本里不存内联副本。 run_problem_file() {   local name="$1" csv="$2" bgfile="$3"   TOTAL=$((TOTAL + 1))   if [ ! -f "$csv" ]; then     echo "[SKIP] $name：数据文件不存在 -> $csv" >&2     FAILED=$((FAILED + 1))     return   fi   echo "================================================================"   echo "=== [$TOTAL] $name"   echo "================================================================"   if "$PYTHON" -m drsr_420.cli.main \       --problem_name "$name" \       --data_csv "$csv" \       --llm_config "$LLM_CONFIG" \       --background_file "$bgfile"; then     echo "[OK] $name"   else     echo "[FAIL] $name（退出码 $?）" >&2     FAILED=$((FAILED + 1))   fi } 
# ── 物理 / 力学 ────────────────────────────────────────────────
...
run_problem_file MRFShear-Cuboid ./data/MRFShear-Cuboid/train.csv \
  backgrounds/MRFShear-Cuboid.txt

run_problem_file MRFShear-Ellipsoid ./data/MRFShear-Ellipsoid/train.csv \
  backgrounds/MRFShear-Ellipsoid.txt
...
run_problem_file MRFCompress-Cuboid ./data/MRFCompress-Cuboid/train.csv \
  backgrounds/MRFCompress-Cuboid.txt

run_problem_file MRFCompress-Ellipsoid ./data/MRFCompress-Ellipsoid/train.csv \
  backgrounds/MRFCompress-Ellipsoid.txt
