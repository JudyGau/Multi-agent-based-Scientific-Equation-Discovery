"""兼容层（@deprecated）：旧路径 ``main`` → 新路径 ``drsr_420.cli.main``。
...
if __package__ in (None, ""):     # 支持 `python 旧路径.py` 直接执行
    ...
    _sys2.path.insert(0, str(_Path(__file__).resolve().parents[0]))

import sys as _sys
import types as _types

from drsr_420.cli.main import *            # noqa: F401,F403  触发子模块导入
from drsr_420.cli import main as _impl
