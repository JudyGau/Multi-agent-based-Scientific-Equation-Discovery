...
- 公式发现智能体，调用文献搜索工具和文献阅读工具获取相关领域的知识，并根据经验总结智能体和残差分析智能体的结果进行公式推导
- 经验总结智能体，经验缓冲区（Experience Buffer）保留更优样本，持续迭代搜索更好结构。
- 残差分析智能体，分析当前公式的预测值与数据集的实际值之间的残差
- 公式解释智能体，对参数拟合后的公式进行解释
...
- 如需切换模型，直接修改 `llm.config` 的相应字段，温度等配置信息也在此处进行修改。
