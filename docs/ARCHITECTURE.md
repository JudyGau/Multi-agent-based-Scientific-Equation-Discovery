...
随仓库分发的绑定里**只有 `summary` 真正换了模型**：`ustc_deepseek-v4-flash`，走下面的
**自定义提供商**路径（见 §10.5）；其余角色共用 `default`（`glm_glm-5.3-flash`）。

`explain` 也占了一行显式绑定，但它当前指向的就是 `default`、参数也等于内置默认值，
所以那一行**不产生任何效果**（等价于省略）。单独绑定这个能力本身保留着——因为"给某个
角色换模型"在旧结构里**原理上无法表达**（见 §8），是否使用由注册表决定。
