...
rem ===========================================================================
rem DrSR 单问题运行示例（Windows 批处理版，与 MRFCompress-Ellipsoid.sh 等价）：
rem 对 MRFCompress-Ellipsoid（压缩模式，7 个样本）执行一次方程发现。
rem
rem 用法：
rem   MRFCompress-Ellipsoid.bat                直接双击，用默认档案与 Python
...
echo ================================================================
echo === MRFCompress-Ellipsoid
echo ================================================================
"%PYTHON%" -m drsr_420.cli.main ^
  --problem_name MRFCompress-Ellipsoid ^
  --data_csv data/MRFCompress-Ellipsoid/train.csv ^
  --num_samplers 4 ^
  --llm_config "%LLM_CONFIG%" ^
  --niterations 6 ^
  --samples_per_iteration 4 ^
  --background_file backgrounds/MRFCompress-Ellipsoid.txt
set "RC=%errorlevel%"
if not "%RC%"=="0" echo [FAIL] MRFCompress-Ellipsoid（退出码 %RC%）
if "%RC%"=="0" echo [OK] MRFCompress-Ellipsoid
call :maybe_pause
exit /b %RC%
