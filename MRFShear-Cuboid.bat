...
rem ===========================================================================
rem DrSR 单问题运行示例（Windows 批处理版，与 MRFShear-Cuboid.sh 等价）：
rem 对 MRFShear-Cuboid（剪切模式，8 个样本）执行一次方程发现。
rem
rem 用法：
rem   MRFShear-Cuboid.bat                      直接双击，用默认档案与 Python
...
echo ================================================================
echo === MRFShear-Cuboid
echo ================================================================
"%PYTHON%" -m drsr_420.cli.main ^
  --problem_name MRFShear-Cuboid ^
  --data_csv data/MRFShear-Cuboid/train.csv ^
  --num_samplers 4 ^
  --llm_config "%LLM_CONFIG%" ^
  --niterations 8 ^
  --samples_per_iteration 4 ^
  --background "Find the mathematical function skeleton that represents magnetorheological effect in shear mode, given data on lambda12(L1/L2), and lambda23(L2/L3). L1, L2, and L3 are the long axis, medium axis, and short axis of the cuboid particle."
set "RC=%errorlevel%"
if not "%RC%"=="0" echo [FAIL] MRFShear-Cuboid（退出码 %RC%）
if "%RC%"=="0" echo [OK] MRFShear-Cuboid
call :maybe_pause
exit /b %RC%
